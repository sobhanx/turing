...
"""Google Meet recording → Turing media connector (OAuth2)."""
...
from turing.connectors.google_meet.oauth import DEFAULT_SCOPES, GoogleMeetOAuthClient
...
def _google_meet_oauth_settings() -> dict[str, str]:
    return {
        "client_id": str(getattr(settings, "TURING_GOOGLE_MEET_CLIENT_ID", "") or ""),
        "client_secret": str(
            getattr(settings, "TURING_GOOGLE_MEET_CLIENT_SECRET", "") or ""
        ),
        "authorize_url": str(
            getattr(settings, "TURING_GOOGLE_MEET_OAUTH_AUTHORIZE_URL", "") or ""
        ),
        "token_url": str(
            getattr(settings, "TURING_GOOGLE_MEET_OAUTH_TOKEN_URL", "") or ""
        ),
        "revoke_url": str(
            getattr(settings, "TURING_GOOGLE_MEET_OAUTH_REVOKE_URL", "") or ""
        ),
        "redirect_uri": str(
            getattr(settings, "TURING_GOOGLE_MEET_OAUTH_REDIRECT_URI", "") or ""
        ),
        "scopes": str(getattr(settings, "TURING_GOOGLE_MEET_OAUTH_SCOPES", "") or "")
        or DEFAULT_SCOPES,
    }
...
class GoogleMeetConnector(BaseConnector):
    """
    Google Meet recordings (Drive) → Turing media (OAuth2).
    ...
    def __init__(
        ...
        *,
        ...
        oauth_client: GoogleMeetOAuthClient | None = None,
    ) -> None:
    ...
    def _oauth(self) -> GoogleMeetOAuthClient:
        if self._oauth_client is not None:
            return self._oauth_client
        cfg = _google_meet_oauth_settings()
        kwargs: dict[str, Any] = {
        ...
        return GoogleMeetOAuthClient(**kwargs)

    def _redirect_uri(self, redirect_uri: str | None = None) -> str:
        uri = (
            (redirect_uri or "").strip()
            or _google_meet_oauth_settings()["redirect_uri"]
        )
        if not uri:
            raise ConnectorConfigurationError(
                "Google Meet OAuth redirect URI is not configured. "
                "Set TURING_GOOGLE_MEET_OAUTH_REDIRECT_URI."
            )
        return uri
    ...
    def authorization_url(
        ...
        return self._oauth().build_authorization_url(
            ...
            state=state_value,
            scopes=_google_meet_oauth_settings()["scopes"],
        )
    ...
    def refresh_credentials(self) -> None:
        ...
        if not refresh:
            ...
            raise AuthenticationError("Google Meet refresh token is missing.")
        try:
            payload = self._oauth().refresh_token(refresh)
        except ConnectorError as exc:
            ConnectorInstallationService().expire(self.installation)
            raise AuthenticationError(
                "Google Meet OAuth token refresh failed.",
                code=getattr(exc, "code", "google_meet_oauth_refresh_failed"),
            ) from exc
        self._persist_token_payload(payload)
    ...
    def revoke_credentials(self) -> None:
        try:
            ...
            logger.warning(
                "Google Meet revoke_credentials failed installation_id=%s",
    ...
    def ensure_fresh_credentials(self) -> None:
        ...
        if expires_at is not None and expires_at <= timezone.now() + _TOKEN_SKEW:
            logger.info(
                "Refreshing Google Meet OAuth token installation_id=%s",
    ...
    def _build_client(self) -> GoogleMeetClient:
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise ConnectorHealthError(
                f"Google Meet health check failed: {exc}"
            ) from exc
        return {
    ...
    def pull_media(self, **kwargs: Any) -> list[MediaPullItem]:
        ...
        for meeting_id, group in by_meeting.items():
            ...
            filename = f"google-meet-{primary.recording_id}.{ext}"
            items.append(
                MediaPullItem(
